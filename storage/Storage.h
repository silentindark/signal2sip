...
// Encrypted (SQLCipher) local storage for one Signal account - one Storage
// instance per signal2sip process, matching the project's one-process-per-
// account design (see memory: project_signal2sip_cpp_rewrite).
class Storage {
public:
    ...
    // ensures the schema exists. Throws std::runtime_error on any failure,
    // including a wrong key against an existing database.
    Storage(const std::string& path, const std::string& key);
    ~Storage();
